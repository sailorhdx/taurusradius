...
def order_form(is_month = True, is_idcard = False, is_address = False):
    ...
    if is_address:
        items.append(btforms.Textbox('address', rules.not_null, description=u'地址信息', required='required', **input_style))
    if is_idcard:
        items.append(btforms.Textbox('idcard', rules.is_idcard, description=u'身份证号码', required='required', **input_style))
        items.append(btforms.Textbox('idcard1_upload', description=u'身份证正面图(人像)', type='button', **input_style))
        items.append(btforms.Textbox('idcard2_upload', description=u'身份证反面图(国徽)', type='button', **input_style))
