...
class IssuesInstallNotifyEvent(BasicEvent):

    def event_service_install(self, account_number, builder_phone = None, wechat_oid = None):
        print account_number
        userinfo = self.get_customer_info(account_number)
        print userinfo
        content_tpl = self.get_content_template(InstallNotify)
        if not content_tpl:
            return
...
def __call__(dbengine = None, mcache = None, **kwargs):
    return IssuesInstallNotifyEvent(dbengine=dbengine, mcache=mcache, **kwargs)