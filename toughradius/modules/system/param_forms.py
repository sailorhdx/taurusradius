...
if os.environ.get('DEMO_VER'):
    ...
    def adconfig_form(**kwargs):
        nodes = kwargs.get('nodes', [])
        return btforms.Form(btforms.Textbox('system_ticket_expire_days', description=u'上网日志保留天数', **input_style), btforms.Textbox('expire_notify_days', rules.is_number, description=u'到期提醒提前天数', **input_style), btforms.Textbox('expire_ipaddr_pool', description=u'到期用户下发地址池', help=u'到期，余额不足用户下发', **input_style), btforms.Dropdown('renew_time_type', args=renew_time_types.items(), description=u'续费模式', **input_style), btforms.Textbox('upgrade_oemid', description=u'OEMID', help=u'OEM升级标识', **input_style), btforms.Dropdown('default_user_node_id', args=nodes, description=u'默认用户区域', help=u'自助开户使用', **input_style), btforms.Textbox('self_recharge_minfee', rules.is_rmb, description=u'自助充值最低金额(元)', help=u'自助门户或微信充值最低金额', **input_style), btforms.Textbox('billtask_last_day', rules.is_number, description=u'用户账单任务截止日期(1-28)', **input_style), btforms.Button('submit', type='submit', html=u'<b>更新</b>', **button_style), title=u'高级参数设置', action='/admin/param/update?active=adconfig')
    ...
    def adconfig_form(**kwargs):
        nodes = kwargs.get('nodes', [])
        return btforms.Form(btforms.Textbox('system_ticket_expire_days', description=u'上网日志保留天数', **input_style), btforms.Textbox('expire_notify_days', rules.is_number, description=u'到期提醒提前天数', **input_style), btforms.Textbox('expire_ipaddr_pool', description=u'到期用户下发地址池', help=u'到期，余额不足用户下发', **input_style), btforms.Dropdown('renew_time_type', args=renew_time_types.items(), description=u'续费模式', **input_style), btforms.Textbox('upgrade_oemid', description=u'OEMID', help=u'OEM升级标识', **input_style), btforms.Dropdown('default_user_node_id', args=nodes, description=u'默认用户区域', help=u'自助开户使用', **input_style), btforms.Textbox('self_recharge_minfee', rules.is_rmb, description=u'自助充值最低金额(元)', help=u'自助门户或微信充值最低金额', **input_style), btforms.Textbox('billtask_last_day', rules.is_number, description=u'用户账单任务截止日期(1-28)', **input_style), btforms.Dropdown('billing_fee_precision', args=fee_precisions.items(), description=u'后付费账单金额精度(元/分)', **input_style), btforms.Dropdown('ssportal_idcard_required', args=booleans.items(), description=u'自助注册开户需要身份证信息', **input_style), btforms.Dropdown('ssportal_address_required', args=booleans.items(), description=u'自助注册开户需要填写地址', **input_style), btforms.Dropdown('ssportal_smsvcode_required', args=booleans.items(), description=u'自助注册开户采用短信验证', **input_style), btforms.Button('submit', type='submit', html=u'<b>更新</b>', **button_style), title=u'高级参数设置', action='/admin/param/update?active=adconfig')